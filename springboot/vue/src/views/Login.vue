<template>
  <div class="wrapper">
    <div style="margin: 220px auto;background-color: #fff;width: 400px;height: 350px;padding: 20px;border-radius: 10px ">
      ...
      <el-form status-icon :model="ruleForm" :rules="rules" ref="ruleForm">
        ...
        <el-form-item prop="password">
          <el-input size="medium" style="margin: 5px 0;" prefix-icon="el-icon-lock" show-password
    ...
    login() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {  //表单校验合法  前端校验通过才开始发请求
          ...
          this.$message.error("验证失败，请确认验证码是否正确")
        }
      })
    }
    ,
